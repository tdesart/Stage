 ...
 *
 */
public class TestS103_S105Transactions {
...
//		Global.logger.info("AbstractMoodleDbWrapper was created as " + db.toString() + ".");
		...
		final S103 rule = new S103(781,"Bachelor en Informatique (professionnel)", "study");
		rule.setEngine(new ProactiveEngine (db, dbPam));

		final boolean ret = rule.execute();
		Global.logger.info("S103.execute() returned: " + ret);
