 ...
 *
 */
public class TestS105Transactions {
...
//		Global.logger.info("AbstractMoodleDbWrapper was created as " + db.toString() + ".");
		...
		final S105 rule = new S105("Master in Mathematics (académique)", "Group Enrolment", "Welcome to your new group!", 50);
		rule.setEngine(new ProactiveEngine (db, dbPam));

		final boolean ret = rule.execute();
		Global.logger.info("S105.execute() returned: " + ret);
