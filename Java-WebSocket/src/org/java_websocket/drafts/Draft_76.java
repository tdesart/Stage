...
public class Draft_76 extends Draft_75 {
	 ...
	 */
	public static byte[] createChallenge( final String key1, final String key2, final byte[] key3 ) throws InvalidHandshakeException {
		final byte[] part1 = getPart( key1 );
		final byte[] part2 = getPart( key2 );
...
//		int max = new Long( maxNumber / spaces ).intValue();
	...
	private static byte[] getPart( final String key ) throws InvalidHandshakeException {
		try {
			final long keyNumber = Long.parseLong( key.replaceAll( "[^0-9]", "" ) );
			final long keySpace = key.split( "\u0020" ).length - 1;
			if( keySpace == 0 ) {
				throw new InvalidHandshakeException( "invalid Sec-WebSocket-Key (/key2/)" );
			}
			final long part = keyNumber / keySpace;
//			final long part = new Long( keyNumber / keySpace );
	...
	public HandshakeState acceptHandshakeAsClient( final ClientHandshake request, final ServerHandshake response ) {
		...
		} catch ( final InvalidHandshakeException e ) {
	...
	public HandshakeBuilder postProcessHandshakeResponseAsServer( final ClientHandshake request, final ServerHandshakeBuilder response ) throws InvalidHandshakeException {
		...
		final String location = "ws://" + request.getFieldValue( "Host" ) + request.getResourceDescriptor();
		response.put( "Sec-WebSocket-Location", location );
		final String key1 = request.getFieldValue( "Sec-WebSocket-Key1" );
		final String key2 = request.getFieldValue( "Sec-WebSocket-Key2" );
		final byte[] key3 = request.getContent();
	...
	public Handshakedata translateHandshake( final ByteBuffer buf ) throws InvalidHandshakeException {
		...
		final HandshakeBuilder bui = translateHandshakeHttp( buf, role );
		...
		if( ( bui.hasFieldValue( "Sec-WebSocket-Key1" ) || role == Role.CLIENT ) && !bui.hasFieldValue( "Sec-WebSocket-Version" ) ) {
			final byte[] key3 = new byte[ role == Role.SERVER ? 8 : 16 ];
			try {
				buf.get( key3 );
