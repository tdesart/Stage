...
public class Draft_75 extends Draft {
	...
	public ByteBuffer createBinaryFrame( final Framedata framedata ) {
		...
		final ByteBuffer pay = framedata.getPayloadData();
		final ByteBuffer b = ByteBuffer.allocate( pay.remaining() + 2 );
	...
	public List<Framedata> createFrames( final String text, final boolean mask ) {
		final FrameBuilder frame = new FramedataImpl1();
		try {
	...
	@Override
	public ClientHandshakeBuilder postProcessHandshakeRequestAsClient( final ClientHandshakeBuilder request ) throws InvalidHandshakeException {
	...
	protected List<Framedata> translateRegularFrame( final ByteBuffer buffer ) throws InvalidDataException {
		while ( buffer.hasRemaining() ) {
			final byte newestByte = buffer.get();
			if( newestByte == START_OF_FRAME ) { // Beginning of Frame
				...
				if( this.currentFrame != null ) {
					currentFrame.flip();
					final FramedataImpl1 curframe = new FramedataImpl1();
		...
		final List<Framedata> frames = readyframes;
	...
	public List<Framedata> translateFrame( final ByteBuffer buffer ) throws InvalidDataException {
		final List<Framedata> frames = translateRegularFrame( buffer );
		if( frames == null ) {
	 ...
	 * @return ByteBuffer 
	 */
	public ByteBuffer increaseBuffer( final ByteBuffer full ) {
		full.flip();
		final ByteBuffer newbuffer = ByteBuffer.allocate( full.capacity() * 2 );
		newbuffer.put( full );
		return newbuffer;
	}
}