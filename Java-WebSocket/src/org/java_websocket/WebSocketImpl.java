 ...
 * text frames, and receiving frames through an event-based model.<br>
 * <br>
 ...
 * <i>onClientMessage</i> callbacks.<br>
...
public class WebSocketImpl extends WebSocket {
	 ...
	 * @param sock 
	 */
	public WebSocketImpl( WebSocketListener listener , final List<Draft> drafts , Socket sock ) {
	 ...
	 * @param sock 
	 */
	public WebSocketImpl( final WebSocketListener listener , Draft draft , final Socket sock ) {
	 ...
	 */
	public void decode( final ByteBuffer socketBuffer ) throws IOException {
		if( !socketBuffer.hasRemaining() || connectionClosed )
			return;

		Global.wssLogger.finer("process(" + socketBuffer.remaining() + "): {" + ( socketBuffer.remaining() > 1000 ? "too big to display" : new String( socketBuffer.array(), socketBuffer.position(), socketBuffer.remaining() ) ) + "}");
	...
	private void decodeFrames( ByteBuffer socketBuffer ) {
		...
		try {
			...
			for( Framedata f : frames ) {
				Global.wssLogger.finer("matched frame: " + f);
	 ...
	 * @param e
	 */
	public void eot( final Exception e ) {
	...
	public void sendFrame( Framedata framedata ) {
		Global.wssLogger.finer("send frame: " + framedata);
	...
	private void write( ByteBuffer buf ) {
		Global.wssLogger.finer("write(" + buf.remaining() + "): {" + ( buf.remaining() > 1000 ? "too big to display" : new String( buf.array() ) ) + "}");
		try {
			outQueue.put( buf );
	...
	private void deliverMessage( Framedata d ) throws InvalidDataException {
		try {
			if( d.getOpcode() == Opcode.TEXT ) {
				...
				Global.wssLogger.finer("Ignoring frame:" + d.toString());
				assert ( false );
			}
	...
	private void open( Handshakedata d ) {
		Global.wssLogger.finer("open using draft: " + draft.getClass().getSimpleName());
