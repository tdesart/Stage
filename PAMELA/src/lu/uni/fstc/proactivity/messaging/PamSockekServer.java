...
import lu.uni.fstc.proactivity.messaging.client.SMClient;
import lu.uni.fstc.proactivity.messaging.client.SMClientAnswerCity;
import lu.uni.fstc.proactivity.messaging.client.SMClientAnswerCountry;
import lu.uni.fstc.proactivity.messaging.client.SMClientCommand;
import lu.uni.fstc.proactivity.parameters.SockectServerConnectionParameters;
 ...
 * 
 */
public class PamSockekServer extends WebSocketServer {
	 ...
	 * @param address
	 */
	public PamSockekServer (final InetSocketAddress address) {
		super (address);
	 ...
	 */
	public PamSockekServer (final int port) throws UnknownHostException {
		super (new InetSocketAddress(port));
		WebSocket.DEBUG = DEBUGGING; // push it forward to the WebSocket
	    Global.wssLogger.config("This Socket Server is now set up in port: " + getPort() +".");
	}
	 ...
	 * @throws Exception 
	 */
	public PamSockekServer (final int port, final AbstractMoodleDbWrapper db, final AbstractPAM2MoodleDbWrapper dbPam, final String configFilePath) throws Exception {
		this(port);
		this.setDb(db);
		this.setDbPam(dbPam);
	...
	public static void main (final String[] args) throws InterruptedException , IOException {
		...
		try {
			...
			s.setDb(MoodleCacheDbWrapper.getInstance("moodle.cfg.xml"));
			s.setDbPam(PAMDbWrapper.getInstance("pam.cfg.xml"));
...
/**/
	 ...
	 * @throws Exception 
	 */
	@edu.umd.cs.findbugs.annotations.SuppressWarnings(value = "OS_OPEN_STREAM", justification = "I wish to ignore this error message. Sandro Reis.")
	public void setSecureConnection(final String configFilePath) throws Exception {
		...
		Global.logger.finer("Secure Connection set from config file = '" + configFilePath + "'");
	}
		...
		synchronized (con) {
		    ...
		    while (itr.hasNext()) {
				...
				if (c.id == id) { // found it !
					...
					// we now don't break, but send to all connections with the same id...
//					break; // go away, no need to continue while cycle
	 ...
	 */
	public final void setDbPam(final AbstractPAM2MoodleDbWrapper dbPam) {
		this.dbPam = dbPam;
	}
